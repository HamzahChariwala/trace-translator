...
# FSDP Configuration
# Note: FULL_SHARD has issues with model.generate() in inference mode
# Using SHARD_GRAD_OP instead - still shows FSDP patterns but more stable
STRATEGY = "fsdp_shard_grad_op"  # SHARD_GRAD_OP - shards gradients and optimizer states
...
if rank == 0:
    print(f"\nLoading {hf_model_name} with FSDP {STRATEGY}")
...
# SHARD_GRAD_OP strategy (more stable for inference):
# - Parameters are replicated (not sharded) - works with model.generate()
# - Gradients and optimizer states would be sharded (if training)
# - Still shows FSDP communication patterns
# - More compatible with HuggingFace generate() method
#
# Note: FULL_SHARD has compatibility issues with model.generate() in inference
model = FSDP(
    model,
    sharding_strategy=ShardingStrategy.SHARD_GRAD_OP,  # More stable for inference
    device_id=torch.cuda.current_device(),
    sync_module_states=True,  # Sync initial states across ranks
    use_orig_params=True,  # Keep original parameters for better compatibility
)
...
if rank == 0:
    print(f"  Model initialized with FSDP {STRATEGY}")
    print(f"  Parameters are replicated across {world_size} GPUs")
    print(f"  Gradients/optimizer states would be sharded (if training)")
    print(f"  Forward pass: Synchronized computation across ranks")
...
if rank == 0:
    ...
    print(f"{'='*60}")
    print(f"Strategy: FSDP {STRATEGY}")
    print(f"  - Parameters replicated across {world_size} GPUs")
    print(f"  - Synchronized forward pass across ranks")
    ...
    print(f"  - Synchronized computation across {world_size} GPUs")
    print(f"  - NCCL Broadcast for parameter synchronization")
    print(f"  - All {world_size} GPUs computing in parallel")
    print(f"  - Barrier synchronization between operations")
    print(f"\nNote: Using SHARD_GRAD_OP instead of FULL_SHARD for stability")
    print(f"      FULL_SHARD has compatibility issues with model.generate()")
    print(f"{'='*60}")
