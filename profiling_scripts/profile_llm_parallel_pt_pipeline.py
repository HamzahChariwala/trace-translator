...
# ============================================================================
# PIPELINE PARALLELISM USING HUGGINGFACE device_map
# ============================================================================
#
# Use HuggingFace's built-in device_map which automatically handles:
# - Layer placement across GPUs
# - Tensor movement between devices
# - Rotary embeddings and other buffers
# - All edge cases in the model architecture
#
# This is the most reliable approach for pipeline parallelism in inference.
# ============================================================================

print(f"  Building custom device_map for {world_size} GPUs...")

# First, load model to get layer count
...
for stage in range(world_size):
    ...
    for layer_idx in range(start_layer, end_layer):
        device_map[f"model.layers.{layer_idx}"] = stage

# Final components on last GPU
last_gpu = world_size - 1
device_map["model.norm"] = last_gpu if hasattr(temp_model.model, 'norm') else 0
device_map["model.final_layernorm"] = last_gpu if hasattr(temp_model.model, 'final_layernorm') else 0
...
# Load model with the custom device_map
# This is the CORRECT way to do pipeline parallelism with HuggingFace
